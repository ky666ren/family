// ===== Family Tree DB (UMD: 浏览器 + Node.js 通用) =====
//
// 浏览器用法（GitHub Pages / 静态托管）：
//   1. <script src="https://cdn.jsdelivr.net/npm/sql.js@1.10.3/dist/sql-wasm.js"></script>
//   2. <script src="./database.js"></script>
//   3. const db = new FamilyTreeDB({ storageKey: 'main' });
//      await db.ensureReady();
//
// Node.js 用法（本地开发 npm start）：
//   const FamilyTreeDB = require('./public/database.js'); // 注意路径
//   const db = new FamilyTreeDB({ dbPath: 'family_tree.db' });
//   await db.ensureReady();
//
// 数据持久化：
//   - 浏览器：通过 IndexedDB 存储 SQLite 文件字节（每个 storageKey 一个）
//   - Node.js：通过文件系统存储 SQLite 文件
...
(function (rootScope, factory) {
  if (typeof module === 'object' && module && module.exports) {
    ...
    rootScope.FamilyTreeDB = exported.FamilyTreeDB;
    rootScope.NodeStorage = exported.NodeStorage;
    rootScope.BrowserStorage = exported.BrowserStorage;
  }
})(typeof globalThis !== 'undefined' ? globalThis : this, function () {
  'use strict';

  // ============ Storage: Node.js (文件系统) ============
  class NodeStorage {
    constructor(dbPath) {
      this._fs = require('fs');
    ...
    async load() {
      if (this._fs.existsSync(this.dbPath)) {
        return new Uint8Array(this._fs.readFileSync(this.dbPath));
      }
      return null;
    }
    async save(bytes) {
      const dir = this._path.dirname(this.dbPath);
      if (!this._fs.existsSync(dir)) this._fs.mkdirSync(dir, { recursive: true });
      this._fs.writeFileSync(this.dbPath, Buffer.from(bytes));
    }
    async remove() {
      if (this._fs.existsSync(this.dbPath)) this._fs.unlinkSync(this.dbPath);
    }
  }

  // ============ Storage: Browser (IndexedDB) ============
  const IDB_NAME = 'family-tree-manager';
  ...
  class BrowserStorage {
    ...
    async load() {
      const bytes = await idbGet(this.key);
      if (!bytes) return null;
      return bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes);
    }
    async save(bytes) {
      const u8 = bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes);
      await idbPut(this.key, u8);
    }
    async remove() { await idbDelete(this.key); }
  }
  ...
  async function loadSqlJs() {
    ...
    if (!globalThis.initSqlJs) {
      throw new Error('initSqlJs 未加载。请先在 HTML 里引入 sql-wasm.js');
    }
  ...
  class FamilyTreeDB {
    constructor(opts = {}) {
      ...
      } else if (opts.dbPath && typeof require === 'function') {
        this.storage = new NodeStorage(opts.dbPath);
      } else if (opts.storageKey) {
        this.storage = new BrowserStorage(opts.storageKey);
      } else if (typeof indexedDB !== 'undefined') {
        this.storage = new BrowserStorage('default');
      } else {
        this.storage = new NodeStorage(opts.dbPath || 'family_tree.db');
      }
      this.db = null;
    ...
    async createFamily(d) {
      ...
      await this.save(); return { id, name: d.name, notes: d.notes||null, description: d.description||null };
    }
    async updateFamily(id, d) {
    ...
    async deleteFamily(id) {
      ...
      this._run('DELETE FROM families WHERE id=?', [id]);
      await this.save();
    }
    ...
    searchCharacters(q) {
      const l = `%${q}%`;
      return this._all('SELECT c.*,f.name as family_name FROM characters c LEFT JOIN families f ON c.family_id=f.id WHERE c.name LIKE ? OR c.identity LIKE ? OR c.occupation LIKE ? ORDER BY c.name', [l, l, l]);
    }
    ...
    async deleteCharacter(id) { this._run('DELETE FROM characters WHERE id=?', [id]); await this.save(); }
    ...
    async deleteMarriage(id) { this._run('DELETE FROM marriages WHERE id=?', [id]); await this.save(); }
    ...
    async deleteBond(id) { this._run('DELETE FROM bonds WHERE id=?', [id]); await this.save(); }
    ...
    async deleteLifeEvent(id) { this._run('DELETE FROM life_events WHERE id=?', [id]); await this.save(); }
    ...
    async createLifeEventCategory(d) {
      const id = this._id();
    ...
    async updateLifeEventCategory(id, d) {
    ...
    async deleteLifeEventCategory(id) {
    ...
    async createTag(d) {
      ...
      await this.save(); return { id, name: d.name, description: d.description||null };
    }
    async updateTag(id, d) {
    ...
    getAncestors(cid, depth) {
      ...
      while (q.length) {
        ...
        if (lvl >= depth || vis.has(id)) continue;
        vis.add(id);
  ...
  return { FamilyTreeDB, NodeStorage, BrowserStorage };
});